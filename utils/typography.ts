import type { CSSRules, Plugin } from "twind";
...
> => ({
  ...
  [className]: (parameters) => {
    ...
    css.forEach((styleList: any) => {
      Object.keys(styleList).forEach((property) => {
        const value = styleList[property];
        const key =
    ...
    parameters.forEach((modifier) => {
      const modifierStyles = styles[modifier as keyof typeof styles].css as any;
      Object.keys(modifierStyles).forEach((property) => {
        const value = modifierStyles[property];
        const key = property;
        rules[key] = value;
      });
    });
